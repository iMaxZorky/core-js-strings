...
function countVowels(str) {
  ...
  str
    ...
    .split('')
    .forEach((element) => {
      if ('aeiouy'.indexOf(element) !== -1) count += 1;
    });
  return count;
}
...
function isPalindrome(str) {
  const pureText = str.replace(/[, !?]/gi, '').toLowerCase();
...
function reverseWords(str) {
  const arr = str.split(' ');
  const result = arr.map((word) => word.split('').reverse().join(''));
  return result.join(' ');
}
...
function encodeToRot13(str) {
  ...
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const cCode = str[i].charCodeAt();
    if (cCode > 64 && cCode < 91) {
      result += encodedChars[cCode - 65];
    } else if (cCode > 96 && cCode < 123) {
      result += encodedChars[cCode - 71];
    } else {
      result += str[i];
    }
  }
